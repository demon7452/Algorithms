...
//        System.out.println(new Test().count("ababccc"));
//        System.out.println(new Test().count("aba"));
        System.out.println(new Test().count("aa"));
    ...
    private static void track(String left) {
        if (left.length() == 0) {
            max = Math.max(max, track.size());
        }
        for (int i = 1; i <= left.length(); i++) {
            String sub = left.substring(0, i);
            String newLeft = left.substring(i);
            track.add(sub);
            track(newLeft);
            track.remove(sub);
        }
